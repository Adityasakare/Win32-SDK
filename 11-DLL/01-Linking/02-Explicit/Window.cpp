...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	HMODULE hLib = NULL;
	typedef int (*LPMAKECUBEFUNCTION)(int);
	LPMAKECUBEFUNCTION pMakeCube = NULL;
	int Num1, Num2;
	TCHAR str[255];
	// Code
	switch (iMsg)
	{
		case WM_CREATE :
			...
			pMakeCube = (LPMAKECUBEFUNCTION)GetProcAddress(hLib, "MakeCube");
			if (pMakeCube == NULL)
			{
				MessageBox(hwnd, TEXT("MakeCube() address cannot obtained"), TEXT("Error"), MB_OK);
				DestroyWindow(hwnd);
			}
			Num1 = 15;
			Num2 = pMakeCube(Num1);
